//Slave as Atmega328p - Arduino Pro Mini
...
// msgs ->    │Wheel(uint16)│EncPos(int32)│RPM(float)│PWM(float)│RefPos(int32)│RefRPM(int32)│ControlFreq(float)│MsgsFreq(float)│
...
//Drive Modes
//Position mode is not used in base control
enum{
  MANUEL_MODE,
  POSITION_MODE,
  VELOCITY_MODE
...
void cmdParser(uint8_t cmd){
  switch(cmd){
    ...
    case CMD_MOVE_TO: {
      ...
      int32_t params[1] = {0};
      memcpy(&params[0], &rdata[1], sizeof(uint32_t));
      m.refPos = params[0];
    } break;
    ...
    case CMD_SET_MODE:{         //8
      uint8_t params[] = {0};
      memcpy(&params[0], &rdata[1], sizeof(uint8_t));
      myMode = params[0];
      motorInit();
    } break;
    ...
    case CMD_GET_POS:   //34
    {
      ...
      memcpy(bufI2C, &posEnc, sizeof(posEnc));
    } break;
    
    case CMD_GET_SPEED: //35
    {
      float RPM = m.RPM/gearRatio;
      memcpy(bufI2C, &RPM, sizeof(RPM));
    } break;
    ...
    case CMD_GET_REF_SPEED: //37
    {
      ...
      memcpy(bufI2C, &refRPM, sizeof(refRPM));
    } break;

    case CMD_GET_REF_POS:
    {
      int32_t refPos = m.refPos;
      memcpy(bufI2C, &refPos, sizeof(refPos));
    } break;
